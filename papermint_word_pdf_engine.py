...
def _convert_with_pandoc(
    ...
    """Fallback for Render's native runtime (Pandoc + PrinceXML)."""
    pandoc = _find_program("pandoc")
    prince = _find_program("prince")
...
""".strip(),
    ...
    environment["PATH"] = (
        str(Path(prince).parent) + os.pathsep + environment.get("PATH", "")
    )
    command = [
        pandoc,
        str(source),
