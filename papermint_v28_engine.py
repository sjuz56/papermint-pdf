...
def convert_v20(pdf_path:Path,out_path:Path,work_root:Path=None,qa=True)->dict:
    ...
    if strategy=='precision':
        # Production/API conversions intentionally run with qa=False.  The old
        # candidate selector still called quality_report_v20 unconditionally,
        # which depends on the development-only /home/oai/skills renderer and
        # made otherwise valid conversions fail on Render.  Use the proven
        # default geometry directly when QA is disabled; the V27/V28 lightweight
        # integrity check still validates the generated DOCX before publishing.
        if not qa:
            convert_precision_v20(
                pdf_path,
                out_path,
                work_root/f'{out_path.stem}_production',
                grid_cols=48,
                font_factor=0.92,
            )
            final={}
            chosen={'grid':48,'font_factor':0.92,'selection':'production-no-qa'}
            result={'version':'V20','strategy':strategy,'router_confidence':confidence,'analysis':analysis,'chosen':chosen,
                    'qa':final,'candidates':[],'output':str(out_path),'seconds_total':round(time.time()-t0,2)}
            return result
