...
TOOLS = [
    ...
    ('pdf-word','PDF to Word','Convert PDF to DOCX in Visual 1:1 or Editable mode.'),
...
async def convert(tool: str = Form(...), files: List[UploadFile] = File(default=[]), mode: str = Form('visual'),
    ...
    try:
        ...
        if tool == 'pdf-word':
            out=pdf_to_word_visual(paths[0]) if mode=='visual' else pdf_to_word_editable(paths[0])
