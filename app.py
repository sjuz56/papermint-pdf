...
async def convert_tool(
    ...
    if tool == "merge":
        ...
        action = "split" if tool == "split" else "compress"
        raise HTTPException(400, f"Please upload exactly one PDF file to {action}.")
    ...
    except (MergeError, SplitError, CompressError) as exc:
