...
TOOLS = [
    ...
    ('pdf-word','PDF to Word','Convert PDF to a fast editable Word document.'),
...
def pdf_to_word_editable(src: Path) -> Path:
    """Fast editable PDF -> DOCX conversion using PyMuPDF.

    This deliberately avoids pdf2docx because that engine can take long enough
    for Render's HTTP connection to time out. Text stays editable; lines are
    reconstructed from their PDF positions and separated with tabs.
    """
    print("FAST PDF WORD STARTED", flush=True)
    pdf = fitz.open(src)
    doc = Document()

    # Narrow margins leave room for wide bank statements/tables.
    ...
    for page_index, page in enumerate(pdf):
        ...
        for block in page_dict.get("blocks", []):
            ...
            for line in block.get("lines", []):
                ...
                lines.append((y0, y1, x0, spans))

        # Group text fragments that visually belong to the same row.
        lines.sort(key=lambda item: (item[0], item[2]))
        rows = []
        for y0, y1, x0, spans in lines:
            cy = (y0 + y1) / 2
            if rows and abs(rows[-1][0] - cy) <= 3.0:
                rows[-1][1].append((x0, spans))
                n = rows[-1][2] + 1
                rows[-1][0] = (rows[-1][0] * rows[-1][2] + cy) / n
                rows[-1][2] = n
            else:
                rows.append([cy, [(x0, spans)], 1])
        ...
        for _, items, _ in rows:
            ...
            previous_right = None
            for item_index, (x0, spans) in enumerate(items):
                if item_index and previous_right is not None:
                    gap = x0 - previous_right
                    p.add_run("\t" if gap > 18 else " ")
                ...
                for sp in spans:
                    ...
                    size = sp.get("size")
                    if size:
                        run.font.size = Pt(max(7, min(float(size), 20)))
                    flags = int(sp.get("flags", 0))
                    run.bold = bool(flags & 16)
                    run.italic = bool(flags & 2)
                    previous_right = sp["bbox"][2]

        # For image-only/scanned pages, insert a clear note instead of running
        # slow OCR inside this endpoint. The separate OCR tool handles scans.
        if not rows:
            p = doc.add_paragraph()
            r = p.add_run("[This page contains no extractable text. Use the OCR tool for scanned PDFs.]")
            r.italic = True
    ...
    doc.save(out)
    pdf.close()
    print("FAST PDF WORD FINISHED", flush=True)
    return out
